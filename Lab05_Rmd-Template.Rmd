...
```{r}
# I have to admit, this solution seems a unnecessarily complicated at first, but I like more flexible solutions much more as they often prevent errors when applied on other datasets...

year_evi_ras <- all_data %>% select(starts_with("y"))

time(year_evi_ras) <- names(year_evi_ras) %>%
  substr(2,5) %>% paste0(., "-07") %>% ym()

ref_2006_2012 <- year_evi_ras %>% 
  subset(
    between(
        time(year_evi_ras),
        as.Date("2006-01-01"),
...
names(ref_2006_2012) <- "reference period 2006-2012"

all_data$EVI_ref_2006_2012 <- ref_2006_2012

# keep your environment clean:
rm(year_evi_ras, ref_2006_2012)
```
...
chaco_df_long %>% 
  ...
  summarise(
    ...
    EVI_q25 = quantile(EVI, probs = .25),
    EVI_q75 = quantile(EVI, probs = .75),
    .groups = "drop"
  ) %>% 
